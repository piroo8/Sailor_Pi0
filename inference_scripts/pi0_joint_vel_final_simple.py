...
def make_robomimic_env(cfg):
    ...
    # 5.4 Confirm final env action contract and apply Sailor wrappers.
...
def _build_droid_example(base_example, obs, prompt, joint, gripper_qpos):
    ...
    # Risk note: assumes these keys exist in env observations.
...
# Input: action with last dim as model gripper logit (DROID-style threshold at 0.5).
# Output: same action shape, but last dim forced to {-1,+1} for RoboMimic/robosuite env.step().
def _binarize_gripper(action):
    action = np.asarray(action).copy()

    # Threshold once in DROID style to obtain a binary value in {0, 1}.
    g_bin = 1.0 if action[-1] > 0.5 else 0.0

    # Convert {0 (open ),1 (closed)} to RoboMimic command range {-1 (open) ,+1 (closed)}.
