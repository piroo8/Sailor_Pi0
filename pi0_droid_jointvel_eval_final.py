...
Behavior:
  - Loads default JOINT_VELOCITY controller config (no normalization fallback).
  - Uses sim-based proprio when explicit obs keys are unavailable.
  - Maps obs to required DROID fields.
  - Runs Sailor ConcurrentEnvs + ModelEvaluator with chunked (10, 8) actions.
"""
...
def _build_droid_example(base_example, obs, prompt, joint, gripper_qpos):
    ...
    if "observation/joint_position" in updated:
        converted = _to_state_like(updated["observation/joint_position"], joint)
    ...
    if "observation/gripper_position" in updated:
        converted = _to_state_like(updated["observation/gripper_position"], g1)
        if converted is None:
            raise ValueError(
